...
适用底层：`com.web3fps.game-foundation` 1.9.1。本文只定义游戏表现和内容，不改变现有安全边界：
...
- v1.9.1 明确保留 v1.7.4 的 RIFT RELAY 宇宙轨道战斗环境；后续功能升级不得把四向轨道废墟背景退化为单色天空或空白平面。
- 六把武器均有可编辑轮廓 Prefab 与 `WeaponDefinition` 数值定义；战斗内仍固定 KESTREL-7，切枪未实现。
- v1.9.1 采用稳定的模块化低多边形角色、程序化行走/后坐力/死亡动作及第一人称手臂；两套 CC0 骨骼士兵 FBX 仅保留为后续替换参考，不由生成场景实例化。
